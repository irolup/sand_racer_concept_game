...
@onready var k_input_handler: Node = $kInputHandler
...
func _ready():
	...
	#spawn_rotation = global_transform.basis
...
func handleInput(delta):
	if Input.is_action_pressed("respawn_car"):
		print("Respawn to",spawn_position )
		respawn()
	k_input_handler.handleInput(delta)

func respawn():
	linear_velocity = Vector3.ZERO
	angular_velocity = Vector3.ZERO
	call_deferred("_apply_spawn_transform")

func _apply_spawn_transform():
	print("Applying spawn:", spawn_position)
	global_transform = Transform3D(spawn_rotation, spawn_position)

func set_respawn(spawn_location: Transform3D) -> void:
	spawn_position = spawn_location.origin + Vector3.UP * 2.0
